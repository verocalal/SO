...
static  int  my_unlink(const  char *path) {
	...
	if ((idxNode = findFileByName(&myFileSystem, (char*) path + 1)) == -1) {
		return -ENOENT; //retorna que no existe el archivo
	}
	int i;
